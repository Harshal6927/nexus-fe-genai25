...
export default function Page() {
  ...
  const [activeSection, setActiveSection] = useState<'matching' | 'features'>(
    'matching',
  )
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveSection((prev) =>
        prev === 'matching' ? 'features' : 'matching',
      )
  ...
  useEffect(() => {
    if (cardSectionRef.current) {
      const matchingSection =
        cardSectionRef.current.querySelector('.matching-section')
      const featuresSection =
        cardSectionRef.current.querySelector('.features-section')
  ...
  return (
    <div className="h-screen overflow-hidden bg-background text-foreground flex flex-col">
      ...
      <header className="py-4 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <h3 className="text-3xl font-bold">
            Connect with perfect candidates through AI-powered analysis of their
      ...
      <div
        className="flex-1 max-w-7xl w-full mx-auto px-4 py-4 relative"
        ref={cardSectionRef}
      >
        {/* Smart Candidate Matching Section - Centered and enhanced */}
        <div
          className="matching-section"
          style={{ display: activeSection === 'matching' ? 'flex' : 'none' }}
        >
                ...
                {[
                  {
                    ...
                    description:
                      'Analyze GitHub, GitLab, and other code repositories',
                  },
                  {
                    icon: UserGroupIcon,
                    label: 'Social Profiles',
                    description:
                      'Review LinkedIn, Twitter and professional networks',
                  },
                  {
                    icon: GlobeAltIcon,
                    label: 'Portfolio Sites',
                    description:
                      'Examine personal websites and project portfolios',
                  },
                  {
                    icon: MagnifyingGlassIcon,
                    label: 'Deep Analysis',
                    description:
                      'Comprehensive skills and cultural fit assessment',
                  },
                ].map((item, index) => (
                  <div
                    key={index}
                    className="flex flex-col p-4 bg-muted rounded-lg border border-border/40 hover:border-primary/40 transition-colors"
                  >
              ...
              {[
                {
                  title: 'Deep Insights',
                  description:
                    'Go beyond resumes with comprehensive analysis of actual skills',
                  color: 'bg-primary/20',
                  icon: BrainCircuit,
                },
                {
                  title: 'Time Saver',
                  description:
                    'Reduce screening time by 80% with automated matching',
                ...
                },
                {
                  ...
                  description:
                    'Proprietary algorithm identifies candidates most likely to succeed',
                  color: 'bg-accent/20',
                  icon: ThumbsUp,
                },
              ].map((feature, index) => (
                <div
                  key={index}
                  className="p-4 rounded-xl text-left border border-border"
                >
                  <div
                    className={`${feature.color} w-10 h-10 rounded-lg mb-3 flex items-center justify-center`}
                  >
